...
FROM employees e
ORDER BY (SELECT department_name FROM departments d WHERE e.department_id = d.department_id) ASC;
...
FROM jobs j,
     (SELECT job_id, AVG(salary) avg_sal
      ...
      GROUP BY job_id
      ORDER BY avg_sal DESC
      LIMIT 1) t_job_max_avg_sal
WHERE j.job_id = t_job_max_avg_sal.job_id;
