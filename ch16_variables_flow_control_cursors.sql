...
# 举例2: 当市场环境变好时，公司为了鼓励大家，决定给大家涨工资。
# 声明存储过程"update_salary_loop()"，声明OUT参数num，输出循环次数。
# 存储过程中实现循环给大家涨薪，薪资涨为原来的1.1倍。直到全公司的
...
BEGIN
    ...
    LOOP
        ...
        # ③ 循环体
        # 如果低于12000，更新员工的工资
        UPDATE employees SET salary=salary * 1.1 WHERE TRUE;
        # ④ 迭代条件
        # 更新avg_sal变量的值
        SELECT AVG(salary) INTO avg_sal FROM employees;
        # 记录循环的次数
...
# 存储过程中实现循环给大家涨薪，薪资涨为原来的1.15倍。知道全公司的平均
# 薪资达到13000结束。并统计循环的次数。
DELIMITER //
...
BEGIN
    ...
    REPEAT
        UPDATE employees SET salary=salary * 1.15 WHERE TRUE;
...
# 举例: 创建存储过程"get_count_by_limit_total_salary()"，声明IN参数limit_total_salary，DOUBLE类型。
...
BEGIN
    ...
    WHILE init_count <= insert_count # ② 循环条件
        DO
            # ③ 循环体
            INSERT INTO admin(user_name, user_pwd)
            VALUES (CONCAT('atguigu-', init_count), ROUND(RAND() * 10000001));
...
# 3. 游标的使用
# 创建存储过程update_salary()，参数1为IN的INT类型变量dept_id，表示部门id;
# 参数2为IN的INT类型变量change_sal_count，表示要调整薪资的员工个数。查询指定id部门的员工信息，
# 按照升序排列，根据hire_date的情况，调整前change_sal_count个员工的薪资，详情如下。
/*
          hire_date                                 salary
